...
- **Extract**: Pulls historical daily data and a 14-day hourly forecast from Open-Meteo. Also fetches air quality data and sunrise/sunset times as additional sources.
...
- "Is the air quality okay?" — air quality alert for sensitive groups, only when needed
...
**Comfort labels instead of raw numbers**: I could have just shown "-12 apparent temperature" and called it done. But that's what every other weather app already does, and most people don't know what "-12 apparent" means in practice. Mapping it to scientifically recognized comfort levels (from the Universal Thermal Climate Index) and translating to everyday language ("Cold — wear layers and a warm jacket") makes the data actually useful.

**apparent_temperature instead of pythermalcomfort**: I investigated the `pythermalcomfort` library for more precise calculations. But Open-Meteo's `apparent_temperature` already factors in wind chill and humidity, so mapping it to the same comfort levels produces the same categories. This removed four heavy dependencies — including a compiler toolchain — and significantly shrank the Docker image. Same outcome for the user, simpler stack.
...
**Cloud Run instead of GKE**: A single dashboard and a periodic pipeline job don't need Kubernetes. Cloud Run shuts down when nobody is using it, so it costs nothing when idle.
...
```bash
make test    # 29 tests (unit + end-to-end integration)
make lint    # ruff lint + format check
```

The tests cover schema validation (does bad data get rejected?), transform logic (do the comfort labels map correctly? do wind labels show up at the right wind speeds?), and a full end-to-end integration test that runs the real pipeline against the live API and verifies the dashboard can read everything back.
